...
The \emph{quality} of remotely-sensed data can be quantified in many ways. For example, the density of a LiDAR point cloud contributes to the power of any statistical derivatives, so the consistency of the point density and accuracy across the campaign is desireable. Hyperspectral imagery can be affected by variations in atmospheric attenuation due to variations in platorm elevation, scale distortions of various kinds, and signal-to-noise fluctuations due to variations in dwell time. As the instruments are generally fixed to the platform, these characteristics must be maintained by careful management of the vehicle's altitude.
...
\begin{itemize}
\item Shot noise \cite{Barducci2007} is due to the natural variability of photon flux -- the number of photons passing through a given area -- which follows a Poisson distribution;
\item thermal noise \cite{Barducci2007}, or dark current \cite{Rogass2011}, results from variability in the number of electrons generated spontaneously within the photosensitive material, as opposed to those activated by the interaction with a photon;
...
\item round-off error \cite{Barducci2007}, or quantization error, which results when real numbers are represented in binary form with limited precision, and;
\item "smearing" \cite{Barducci2007,Ruyten1999}, which occurs as the photosensor remains active during charge transfer.
\end{itemize}

These sources have in common that they are not easily manageable by the user, but intrinsic to the instrument. However, by increasing the magnitude of the signal, the relative influence of noise can be reduced \cite{Dor2012}. Of course, the strength of the signal is determined by several factors, including atmospheric attenuation and irradiant intensity, the optical properties of the instrument and the reflectivity of the target, which cannot be directly controlled, and several that can. The most direct way to increase the strength of the signal is to extend the time interval during which photons are collected by the instrument, the \emph{integration time}. 

Because instrument is generally in motion, the \emph{integration time} must be carefully considered in relation to the platform altitude and velocity. For an ideal push-broom spectrometer, with an along-track IFOV of zero, the integration time and velocity determine the along-track spatial resolution. That is, at $\SI{1}\metre/\second$, with an integration time of $\SI{1}\second$, the spatial resolution is 1m. 


[Fix the above gibberish.]
...
\item $d_{\lambda}$ -- the \emph{dark current};
