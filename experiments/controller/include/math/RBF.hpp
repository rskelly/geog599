...
public:
	...
	double fn(double t, double u, double sigma2) {
		return std::exp(-std::pow(t - u, 2.0) / sigma2);
	}
	...
	bool fit(const std::vector<P>& pts, double sigma, double spacing) {
		...
		for(size_t i = 0; i < pts.size(); ++i) {
			const P& pt = pts[i];
			size_t j1 = (size_t) std::max(0L, (long) std::floor((pt.y() - m_xmin - sigma * 3) / spacing));
			size_t j2 = std::min(x.size(), (size_t) std::ceil((pt.y() - m_xmin + sigma * 3) / spacing));
			for(size_t j = j1; j < j2; ++j) {
				double f = fn(x[j], pt.y(), sigma2);
				m_output[j].y(x[j]);
				m_output[j].z(f * pt.z());
			}
		}
