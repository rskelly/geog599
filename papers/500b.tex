...
This last point reintroduces the discussion of instrument and algorithm selections. A spectrometer (i.e., an RGB camera) with a wide-angle lens will cover the desired field of view at high resolution and low cost, but will require computationally-intensive photogrammetric or optical-flow processing. A wide-swath laser scanner, such as the Velodyne VLP32 provides both a wide swath, and sufficient spread that, if mounted vertically, would provide the desired coverage. Finally, one or more single-beam laser rangefinders could be arranged to sample the space between nadir and the forward-facing laser.
